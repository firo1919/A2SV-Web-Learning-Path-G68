...
async function JobDescription({ params }: { params: Promise<{ jobId: string }> }) {
	...
	const jobPost = await JobPostService.getJobPost(jobId);
	...
	const responsibilities = jobPost.responsibilities.split(".").filter((j) => j.trim() !== "");
	const idealCandidate = jobPost.idealCandidate.split(".").filter((c) => c.trim() !== "");
	return (
		<div className="flex gap-[62px]">
			...
			<div className="flex-1/4 shrink-0 flex flex-col gap-5">
				...
				<div className="">
					...
					<div className="flex flex-wrap gap-2">
						{jobPost.requiredSkills.map((skill, id) => (
